...
using MicroHomeAssistantClientTests.Json;
...
public class HassMockStartup : IHostedService
{
    ...
    private JsonElementSerializationContext _jsonElementSerializationContext = new();
    ...
    private static async Task ProcessRequest(HttpContext context)
    {
        ...
        await context.Response.WriteAsJsonAsync(
            new 
            {
                ...
                device_id = "ksakksk22kssk2",
                area_id = "ssksks2ksk3k333kk",
                name = "name"
            }
    ...
    private async Task ProcessWebsocket(WebSocket webSocket)
    {
        ...
        try
        {
            ...
            while (true)
            {
                ...
                var hassMessage =
                    (JsonElement?) JsonSerializer.Deserialize(new ReadOnlySpan<byte>(buffer, 0, result.Count), typeof(JsonElement), _jsonElementSerializationContext)
                     ?? throw new ApplicationException("Unexpected not able to deserialize");

                var messageType = HaMessageHelper.GetHaMessageType(hassMessage);
                
                var messageId = messageType != "auth" ? HaMessageHelper.GetMessageId(hassMessage) : 0;
                
                switch (HaMessageHelper.GetHaMessageType(hassMessage))
                {
